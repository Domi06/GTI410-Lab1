...
public class ColorDialog extends JDialog {
	...
	private JPanel createCMYKPanel(ColorDialogResult result, int imageWidths) {	
		...
		//calcul des valeurs en CMYK de result pour initialiser les curseurs
		int red=result.getPixel().getRed();
		int green=result.getPixel().getGreen();
		int blue=result.getPixel().getBlue();
				
		int cyan = cmykMediator.getCyan(red, green, blue);
		int magenta = cmykMediator.getMagenta(red, green, blue);
		int yellow = cmykMediator.getYellow(red, green, blue);
		int black = cmykMediator.getBlack(red, green, blue);
		
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		ColorSlider csCyan = new ColorSlider("C:", cyan, cmykMediator.getCyanImage());
		ColorSlider csMagenta = new ColorSlider("M:", magenta, cmykMediator.getMagentaImage());
		ColorSlider csYellow = new ColorSlider("Y:", yellow, cmykMediator.getYellowImage());
		ColorSlider csBlack = new ColorSlider("K:", black, cmykMediator.getBlackImage());
	...
	private JPanel createHSVPanel(ColorDialogResult result, int imageWidths) {
		...
		int hue = (int)(hsvMediator.getHue(red, green, blue));
		int saturation = (int)(hsvMediator.getSaturation(red, green, blue));
		int value = (int)(hsvMediator.getValue(red, green, blue));
		
		JPanel panel = new JPanel();
		panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
		ColorSlider csHue = new ColorSlider("H:", hue, hsvMediator.getHueImage());
		ColorSlider csSaturation = new ColorSlider("S:", saturation, hsvMediator.getSaturationImage());
		ColorSlider csValue = new ColorSlider("V:", value, hsvMediator.getValueImage());
